  ...
  function tryRead() {
    ...
    if (raw !== undefined) {
      try {
        ...
        // Remember which account this cached snapshot belongs to. It was
        // painted speculatively (to avoid a 0% flash) before we know which
        // account is actually signed in on this load — network.js compares
        // this against the freshly-detected orgId the moment it's known,
        // and clears the cache if they don't match (e.g. a fresh account
